...
class VCFFilter:
    ...
    def _genotype_filter(self, input_vcf: Path, gq: int, ad_binom: float, snp_depth: int, indel_depth: int, wes: bool) -> Path:
        ...
        cmd = f'bcftools filter -Ob -o {output_vcf} --threads 4 -S . -i {filtering_string} {input_vcf}'
    ...
    def _set_missingness_values(self, input_vcf: Path) -> Path:
        ...
        cmd = f'bcftools +fill-tags {input_vcf} -Ob ' \
              f'-o {output_vcf} -- -t \'F_MISSING,AC,AF,AN,GTM=count(FORMAT/GT == "./."),GT0=count(FORMAT/GT == "0/0"),GT1=count(FORMAT/GT == "0/1"),GT2=count(FORMAT/GT == "1/1")\''
        self._cmd_executor.run_cmd_on_docker(cmd)
    ...
    def _write_index(self, input_vcf: Path) -> Path:
        ...
        cmd = f'bcftools index --threads 4 {input_vcf}'
        self._cmd_executor.run_cmd_on_docker(cmd)
