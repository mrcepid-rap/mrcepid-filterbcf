...
class VCFAnnotate:
    ...
    def _run_vep(self, input_vcf: Path) -> Path:
        ...
        # Explicit mount: pytest tempdir (or local "test") → /test inside container
        test_mount = DockerMount(
            local=Path.cwd().resolve(),  # pytest tempdir (or "test" locally)
            remote=Path("/test")
        )

        # Container-side paths only
        loftee_path = Path("/test/loftee_files/loftee_hg38")
        ...
        fasta_path = Path("/test/reference.fasta")
        vep_caches_path = Path("/test/vep_caches")

        # Quote all paths with shlex.quote to avoid Docker argument errors
