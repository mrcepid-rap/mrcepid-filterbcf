...
class VCFAnnotate:
    ...
    def __init__(self, vcf_path: Path, filtered_vcf: Path, additional_annotations: List[AdditionalAnnotation],
        ...
        self._cmd_executor = CommandExecutor(docker_image="egardner413/mrcepid-burdentesting:v0")
        ...
        self.finalbcf = exporter.export_files(filtered_vcf)
        self.finalbcf_index = exporter.export_files(f'{filtered_vcf}.csi')
        self.finalvep = exporter.export_files(vep_gz)
        self.finalvep_index = exporter.export_files(vep_gz_tbi)
    ...
    def _run_vep(self, input_vcf: Path) -> Path:
        ...
        # 1. We do NOT need to manually mount Docker directories.
        # The CommandExecutor AUTOMATICALLY mounts the current working directory (where your files are)
        # to '/mnt/host_cwd' inside the container.

        # 2. We define paths relative to this internal mount point.
        # Since 'loftee_files', 'vep_caches', etc., are in your local CWD,
        # they will be at /mnt/host_cwd/ inside Docker.
        container_base = Path("/mnt/host_cwd")

        loftee_path = container_base / "loftee_files/loftee_hg38"
        human_ancestor_fa = loftee_path / "human_ancestor.fa.gz"
        conservation_file = loftee_path / "loftee.sql"
        gerp_file = loftee_path / "gerp_conservation_scores.homo_sapiens.GRCh38.bw"

        fasta_path = container_base / "reference.fasta"
        vep_caches_path = container_base / "vep_caches"

        # Helper to quote paths safely
        quoted = lambda p: shlex.quote(str(p))

        # 3. Construct the command using these explicit container paths.
        # Note: We pass 'input_vcf' (the Host Path) directly to -i.
        # The CommandExecutor is smart enough to rewrite simple arguments like -i automatically.
        # However, it CANNOT rewrite the complex comma-separated --plugin string,
        # which is why we manually used 'container_base' variables above.
        cmd = (
            f'perl -I/ensembl-vep/cache/Plugins/loftee/ '
            f'-I/ensembl-vep/cache/Plugins/loftee/maxEntScan/ '
            f'/ensembl-vep/vep --offline --cache --assembly GRCh38 '
            f'--dir_cache {quoted(vep_caches_path)} --everything --allele_num --fork 4 '
            f'-i {quoted(input_vcf)} --format vcf --fasta {quoted(fasta_path)} '
            f'-o {quoted(output_vcf)} --compress_output bgzip --vcf '
            f'--dir_plugins /ensembl-vep/cache/Plugins/ '
            f'--plugin LoF,loftee_path:/ensembl-vep/cache/Plugins/loftee,'
            f'human_ancestor_fa:{quoted(human_ancestor_fa)},'
            f'conservation_file:{quoted(conservation_file)},'
            f'gerp_bigwig:{quoted(gerp_file)} --force_overwrite'
        )

        # 4. Run the command.
        # We MUST keep protected_container_paths to stop it from overwriting the internal /ensembl-vep folder.
        # We REMOVED the manual docker_mounts=[test_mount] argument.
        self._cmd_executor.run_cmd_on_docker(
            cmd=cmd,
            protected_container_paths=["/ensembl-vep"]
        )
    ...
    def _add_additional_annotation(self, input_tsv, annotation: AdditionalAnnotation) -> Tuple[Path, str]:
        ...
        * This method is hard-coded to use gene SYMBOL as the matching field. This is because gene SYMBOL is the most
        ...
        cmd = f'tabix --threads 4 -h {annotation["file"]} {self.chunk_chrom}:{self.chunk_start - 1}-{self.chunk_end}'
        self._cmd_executor.run_cmd_on_docker(cmd, stdout_file=Path(sliced_tsv))

        sliced_bgzip, _ = bgzip_and_tabix(sliced_tsv, comment_char='#', end_row=2, force=True)
        ...
        cmd = f'annot-tsv ' \
              ...
              f'-o {output_tsv}'
        self._cmd_executor.run_cmd_on_docker(cmd)
        # Clean up, unless we are running pytest
    ...
    def _parse_vep(self, raw_vep: Path, annotation_names: List[str]) -> Tuple[Path, Path]:
        ...
        vep_gz, vep_gz_idx = bgzip_and_tabix(annote_file, comment_char='C', end_row=2, force=True)
