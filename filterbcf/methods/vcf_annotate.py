...
class VCFAnnotate:
    ...
    def _run_vep(self, input_vcf: Path) -> Path:
        ...
        # Use real, resolved host paths so mounting works
        ...
        fasta_path = Path("reference.fasta").resolve()
        vep_caches_path = Path("vep_caches").resolve()

        # The VEP command string. The command executor will rewrite the host paths
        # from the force_include_paths list into container paths.
        cmd = (
