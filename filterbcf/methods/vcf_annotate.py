...
class VCFAnnotate:
    ...
    def _run_vep(self, input_vcf: Path) -> Path:
        ...
        # Use real, resolved paths so mounting works
        loftee_path = Path("loftee_files/loftee_hg38").resolve()
        human_ancestor_fa = (loftee_path / "human_ancestor.fa.gz").resolve()
        conservation_file = (loftee_path / "loftee.sql").resolve()
        gerp_file = (loftee_path / "gerp_conservation_scores.homo_sapiens.GRCh38.bw").resolve()
        ...
        cmd = (
            ...
            f'--dir_cache vep_caches/ --everything --allele_num --fork 4 '
            f'-i {input_vcf} --format vcf --fasta reference.fasta '
            ...
            f'human_ancestor_fa:loftee_files/loftee_hg38/human_ancestor.fa.gz,'
            f'conservation_file:loftee_files/loftee_hg38/loftee.sql,'
            f'gerp_bigwig:loftee_files/loftee_hg38/gerp_conservation_scores.homo_sapiens.GRCh38.bw'
        )

        self._cmd_executor.run_cmd_on_docker(
            cmd=cmd,
            force_include_paths=[loftee_path, human_ancestor_fa, conservation_file, gerp_file]
        )
    ...
    def _add_additional_annotation(self, input_tsv, annotation: AdditionalAnnotation) -> Tuple[Path, str]:
        ...
        * This method is hard-coded to use gene SYMBOL as the matching field. This is because gene SYMBOL is the most
        ...
        cmd = f'tabix --threads 4 -h {annotation["file"]} ' \
