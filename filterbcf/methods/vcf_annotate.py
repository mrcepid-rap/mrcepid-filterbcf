...
class VCFAnnotate:
    ...
    def _run_vep(self, input_vcf: Path) -> Path:
        """
        Runs the Variant Effect Predictor (VEP) with specific configurations for annotating a VCF file.

        This method constructs and executes a command for running VEP within a specified containerized
        environment. The method uses container paths for various resources, ensuring proper handling
        of system paths and protections during execution.

        Due to the nature of VEP and the convoluted paths, we have to be very careful about how we construct
        the command.

        :param input_vcf: Path to the input VCF file to annotate.
        :return: Path to the annotated VCF file.
        """
        output_vcf = Path(f'{self.vcfprefix}.sites.vep.vcf.gz')

        # 1. Define paths assuming the standard mount point (/mnt/host_cwd)
        ...
        # 2. Construct the command using these explicit container paths.
        cmd = (
        ...
        # 3. Protect BOTH the internal VEP dir AND our manual mount point.
        # This tells the executor: "If you see a path starting with /mnt/host_cwd,
        # DO NOT try to mount it and DO NOT rewrite it."
        self._cmd_executor.run_cmd_on_docker(
            cmd=cmd,
            protected_container_paths=["/ensembl-vep", "/mnt/host_cwd"]
        )
