...
class VCFAnnotate:
    ...
    def _run_cadd(self, cadd_vcf: Path) -> AdditionalAnnotation:
        ...
        # CADD requires specialised mounts in Docker to run properly. I spoof the default paths that CADD expects as
        # mount points within the image.
        ...
        self._cmd_executor.run_cmd_on_docker(cmd, docker_mounts=cadd_mounts)
    ...
    def _add_additional_annotation(self, input_vcf: Path, annotation: AdditionalAnnotation) -> Tuple[Path, str]:
        ...
        INFO field of the resulting bcf.
        ...
        cmd = f'bcftools annotate --threads 2 ' \
              f'-a /test/{annotation["file"]} ' \
              f'-c CHROM,POS,REF,ALT,{annotation["annotation_name"]} -Ob ' \
